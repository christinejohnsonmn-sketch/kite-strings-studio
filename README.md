...
- `js/list-block.js` — three shared, reusable list components: `KSDListBlock` (plain add/remove list), `KSDTaskList` (checkbox + reorder + optional due-date auto-sort, plus an optional "→ Today" button that moves an item straight into that business's actual-today Upcoming list), and `KSDStaticWeekly` (fixed recurring checklist that resets every Monday) — used across Studio Wall and every Room page.
