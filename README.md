...
- `js/list-block.js` — three shared, reusable list components: `KSDListBlock` (plain add/remove list), `KSDTaskList` (checkbox + reorder + optional due-date auto-sort), and `KSDStaticWeekly` (fixed recurring checklist that resets every Monday) — plus `KSDPushToToday`, a shared helper that sends an item's text into a business's actual-today Upcoming list. Every list on every Room board (backlog lists, recurring checklists, and the KSD Client board's project/proposal tasks) has a "→ Today" button. Backlog-style lists (Website, Misc, Content Ideas, Business Development, Future Ideas, etc.) move the item — it leaves the source list. Lists where a checkbox elsewhere is the real completion tracker (static weekly checklists, and the Client board's phase/proposal tasks) copy instead, leaving the original in place.
